...
export function ServiceDetailView({ serviceId }: ServiceDetailViewProps) {
  ...
  if (!service) {
    return (
      <View className="flex-1 items-center justify-center bg-background">
        <Text className="text-lg text-muted-foreground font-mbold">Không tìm thấy dịch vụ</Text>
  ...
  const handleBookPress = () => {
    console.log('Navigate to booking for service:', service.id, 'provider:', service.providerId);
  };
  ...
  return (
    <View className="flex-1 bg-background relative">
      ...
      <View className="absolute top-0 left-0 right-0 z-10">
        ...
        />
        <SafeAreaView edges={['top']} className="px-4 pt-1 pb-2 flex-row items-center">
          <Pressable
            onPress={() => router.back()}
            className="w-10 h-10 bg-white/80 rounded-full flex items-center justify-center shadow-sm active:bg-white"
          >
